...
def IdentityNet(x):
    # h_conv1: 112*112*32
    bn1_identitynet = tf.layers.batch_normalization(conv2d(x, W_conv1_identitynet), training = is_training)
    h_conv1_identitynet = tf.nn.relu(bn1_identitynet + b_conv1_identitynet)
    # h_pool1: 56*56*32
    h_pool1_identitynet = max_pool_2x2(h_conv1_identitynet)
    # h_conv2: 56*56*64
    
    bn2_identitynet = tf.layers.batch_normalization(conv2d(h_pool1_identitynet, W_conv2_identitynet), training = is_training)
    h_conv2_identitynet = tf.nn.relu( bn2_identitynet+ b_conv2_identitynet)
    # h_pool2: 28*28*64
    h_pool2_identitynet = max_pool_2x2(h_conv2_identitynet)
    # h_conv3: 28*28*128
    
    bn3_identitynet = tf.layers.batch_normalization(conv2d(h_pool2_identitynet, W_conv3_identitynet), training = is_training)
    h_conv3_identitynet = tf.nn.relu( bn3_identitynet+ b_conv3_identitynet)
    # h_pool3: 14*14*128
...
def PoseNet(x):
    # h_conv1: 112*112*32
    bn1_posenet = tf.layers.batch_normalization(conv2d(x, W_conv1_posenet), training = is_training)
    h_conv1_posenet = tf.nn.relu(bn1_posenet + b_conv1_posenet)
    # h_pool1: 56*56*32
    h_pool1_posenet = max_pool_2x2(h_conv1_posenet)
    # h_conv2: 56*56*64
    
    bn2_posenet = tf.layers.batch_normalization(conv2d(h_pool1_posenet, W_conv2_posenet) , training = is_training)
    h_conv2_posenet = tf.nn.relu(bn2_posenet+ b_conv2_posenet)
    # h_pool2: 28*28*64
    h_pool2_posenet = max_pool_2x2(h_conv2_posenet)
    # h_conv3: 28*28*128
    
    bn3_posenet = tf.layers.batch_normalization(conv2d(h_pool2_posenet, W_conv3_posenet), training = is_training)
    h_conv3_posenet = tf.nn.relu( bn3_posenet+ b_conv3_posenet)
    # h_pool3: 14*14*128
...
loss_dis=-tf.reduce_sum(y_dis*tf.log(tf.maximum(dis,1e-10))+(1-y_dis)*tf.log(tf.maximum(1-dis,1e-10)))
    ...
    for pre in range(0,50):
        ...
        for i in range(0,epochs):
            ...
            if (i+1)*batch_size<N_t:
                ...
                _,b=sess.run([train,loss],feed_dict={sample1: x_t_seg, sample2: x_s_seg,
                                                     y_sample1: t_label_one_hot_seg,y_sample2: s_label_one_hot_seg,
                                                     res_sample1:x_t_res_seg, res_sample2:x_s_res_seg,
                                                     y_dis:similar_seg,
                                                     is_training:True})
        ...
        if epoch==epoch:   #(epoch+1) % 100 ==0
            ...
            for i in range(0,epochs_test):
                if (i+1)*batch_size<x_test.shape[0]:
                    ...
                    cr_temp=np.sum(sess.run(correct_prediction, feed_dict={sample1: x_test[i*batch_size:(i+1)*batch_size,:,:,:],
                                                                           y_sample1: test_label_one_hot[i*batch_size:(i+1)*batch_size,:],
                                                                           is_training:False}))
                    pred_test=np.vstack((pred_test,sess.run(sample1_out, feed_dict={sample1: x_test[i*batch_size:(i+1)*batch_size,:,:,:]})))
                    cr=cr+cr_temp
                else:
                    cr_temp=np.sum(sess.run(correct_prediction, feed_dict={sample1: x_test[i*batch_size:,:,:,:],
                                                                           y_sample1: test_label_one_hot[i*batch_size:,:],
                                                                           is_training:False}))
                    pred_test=np.vstack((pred_test,sess.run(sample1_out, feed_dict={sample1: x_test[i*batch_size:(i+1)*batch_size,:,:,:]})))           
                    cr=cr+cr_temp
